...
def train(args):
    ...
    if args.use_pretrain_model:
        ...
        if hvd_rank == 0:
            logging.info('loaded pretrain model: {}'.format(args.pretrain_model_path))
            print('{} loaded pretrained parameters:'.format(len(pretrained_key)))
            for k in pretrained_key:
                print('\t{}'.format(k))
            print('{} randomly initialized parameters:'.format(len(remain_key)))
            for k in remain_key:
                print('\t{}'.format(k))
        ...
        for index, layer in enumerate(model.news_encoder.bert_model.encoder.layer):
            if index in args.bert_trainable_layer:
                logging.info("finetune block {}".format(index))
    ...
    if args.load_ckpt_name is not None:
        ...
        logging.info("Model loaded from {}".format(ckpt_path))
    ...
    for ep in range(args.start_epoch, args.epochs):
        ...
        if hvd_rank == 0:
            ckpt_path = os.path.join(args.model_dir, 'epoch-{}.pt'.format(ep+1))
            torch.save(
                {
            ...
            logging.info("Model saved to {}".format(ckpt_path))
